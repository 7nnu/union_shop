...
class _SaleCollectionPageState extends State<SaleCollectionPage> {
  ...
  List<Product> _computeList() {
    List<Product> list = sampleSaleProducts.toList();

    if (_filter == 'Beanies') {
      list =
          list.where((p) => p.title.toLowerCase().contains('beanie')).toList();
    } else if (_filter == 'Hoodies') {
      list =
          list.where((p) => p.title.toLowerCase().contains('hoodie')).toList();
    } else if (_filter == 'T-Shirts') {
      list = list
          .where((p) =>
              p.title.toLowerCase().contains('t-shirt') ||
              p.title.toLowerCase().contains('tshirt'))
          .toList();
    } else if (_filter == 'Jackets') {
      list =
          list.where((p) => p.title.toLowerCase().contains('jacket')).toList();
    }
  ...
  Widget build(BuildContext context) {
    ...
    final labelStyle =
        TextStyle(fontSize: isMobile ? 10 : 12, color: Colors.grey);
